...
@interface NetworkGuideView ()<UIScrollViewDelegate>

@property (nonatomic,strong)NSArray *imageArr;//图片数组,存放的图片链接地址字符串
@property (nonatomic,strong)NSString *placeholderImage;//无图片显示的占位图片
...
- (instancetype)initWithFrame:(CGRect)frame and:(NSArray *)imgArr andPlaceholderImage:(NSString *)imageName{
    if (self = [super initWithFrame:frame]) {
        ...
        self.placeholderImage = imageName;
...
- (void)loadImageForScrollViewWithFrame:(CGRect)frame{
    ...
    UIImage *image = [UIImage imageNamed:self.placeholderImage];
